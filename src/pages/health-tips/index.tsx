...
import { getHealthTips, deleteHealthTipById } from 'apiSdk/health-tips';
...
function HealthTipListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('health_tip', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/health-tips/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('health_tip', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/health-tips/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('health_tip', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/health-tips/edit/${record.id}`} passHref legacyBehavior>
                          <Button
